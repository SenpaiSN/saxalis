...
export default function FinancialHealthCard({
  ...
  // Calcul de l'épargne long terme (Natixis, bloquée 5 ans)
  function getEpargneLongTerme() {
    if (!transactions || !Array.isArray(transactions)) return 0;
    return transactions.filter(t => {
      const type = String(t.type || '').toLowerCase();
      const cat = String(t.categorie || '').toLowerCase();
  ...
  // Include ONLY liquid savings in emergency fund calculation (exclude long-term savings like Natixis)
  const epargneLongTerme = getEpargneLongTerme();
  const epargneLiquideSaved = epargneReal - epargneLongTerme;
  const emergencyFundMonths = healthData.avgMonthlyExpenses > 0 
    ? Math.round(((soldeReal + epargneLiquideSaved) / healthData.avgMonthlyExpenses) * 10) / 10 
    : 0;
  
  // Total patrimoine
  const totalPatrimoine = soldeReal + epargneReal;
        ...
        <div className="flex gap-6 mb-6">
          ...
          <div className="flex-1">
            ...
            <div className="space-y-2">
              ...
              {/* Ajout : détail de l'épargne */}
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-500">Dont épargne</span>
                <span className="text-xs font-semibold text-indigo-600">
                  {formatCurrency(healthData.versementsObjectifsEnCours || 0)}
                </span>
              </div>
              <div className="flex items-center justify-between">
