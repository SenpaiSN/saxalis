...
export default function StatsModern({ transactions, recherche, setRecherche, annee, setAnnee, mois, setMois, filtreType, setFiltreType, categorie, setCategorie, sousCategorie, setSousCategorie, types, categories, subcategories, categoriesLoading, subcategoriesLoading, anneesDisponibles, resetFilters }: StatsModernProps) {
  ...
  // Clé unique pour ce module (adapter si plusieurs pages similaires)
  const STORAGE_KEY = 'statsmodern-filters';
  const STORAGE_TAB_KEY = 'statsmodern-periode';

  // Initialisation des filtres depuis localStorage
  useEffect(() => {
    try {
    ...
    try {
      ...
      if (tab) setPeriode(tab as 'semaine'|'mois'|'annee');
    } catch {}
  ...
  // Note: do NOT rely on delayed wrappers for Reference* components.
  // The correct approach is to render ReferenceDot/ReferenceArea only when axes/data are stable
  // (see guards below) and to pass explicit xAxisId/yAxisId props.
  ...
  const statsItems = [
    {
      title: 'Solde total',
      value: formatEuro(currentSolde),
      change: soldeChangeLabel,
      comparison: `Total (incl. prévisionnel): ${formatEuro(totals.resteADepenserAll)}`,
      trend: (currentSolde >= prevSolde) ? 'up' : 'down',
      icon: Wallet,
    ...
    {
      title: 'Revenus',
      value: formatEuro(totalRevenusReal),
      change: revenusChangeLabel,
      comparison: revenusChangeLabel,
      trend: (totalRevenusReal >= prevRevenus) ? 'up' : 'down',
      icon: TrendingUp,
    ...
    {
      title: 'Dépenses',
      value: formatEuro(totalDepensesReal),
      change: depensesChangeLabel,
      comparison: depensesChangeLabel,
      trend: (totalDepensesReal >= prevDepenses) ? 'up' : 'down',
      icon: TrendingDown,
  ...
  // When computed data changes, briefly wait one tick so Recharts has time to register axes
  useEffect(() => {
  ...
  const monthlySavings = computeMonthlySavingsAndProjections(transactionsFiltres); // returns trimmed series with projections
  ...
  const formatEuro = (v: number) => new Intl.NumberFormat(locale, { style: 'currency', currency }).format(v);

  // Goals summary (fetched from API)
  const [goalsList, setGoalsList] = useState<Array<any>>([]);
  const [goalsLoading, setGoalsLoading] = useState(false);
  const [goalsError, setGoalsError] = useState<string | null>(null);

  // Charts readiness guard: avoid drawing Reference overlays before axes are registered
  ...
  // Apply YEAR filter only for display (do NOT recompute cumul)
  // Keep projection months (proj !== undefined) visible regardless of selected year so users always see the 3-month projection
  if (annee && annee !== 'Tous') {
  ...
  // compute ticks for savings chart (show all or a subset)
  const savingsTickStep = Math.max(1, Math.ceil(savingsChartData.length / 6));
  const savingsTicksIndices = savingsChartData.map((s, i) => (i % savingsTickStep === 0 ? i : null)).filter(v => v !== null) as number[];
  // guard: ensure axis/data stability before rendering reference overlays
  const hasValidSavingsAxis = savingsChartData.length > 1 && savingsTicksIndices.length > 0 && typeof (savingsChartData[0] as any)?.index === 'number';

  // when computed data sizes change, ensure charts recalc their measurements
  useEffect(() => {
    ...
    <div className="p-3 lg:p-6 space-y-4" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 1.5rem)' }}>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="col-span-1 lg:col-span-1 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <p className="mt-3 text-sm font-semibold text-gray-700">Total épargné sur objectifs: <strong>{formatEuro(totalSavedGoals)}</strong></p>
        </div>
      ...
      <WithdrawFromGoalModal open={withdrawOpen} goal={activeGoal} onClose={() => setWithdrawOpen(false)} onDone={() => loadGoals()} categories={categories} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="h-48">
            <EvolutionChart data={evolutionData} formatCurrency={formatEuro} chartsReady={chartsReady} locale={locale} />
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="h-48">
            <CategoryChart data={categoriesData} formatCurrency={formatEuro} colors={COLORS} />
          </div>
        </div>
      </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="px-3 py-1 rounded-full text-sm font-medium" style={{ backgroundColor: 'var(--card-bg-epargne)', color: 'var(--color-epargne-foreground)' }}>
                  {formatEuro(projSumValue)}
                </span>
              </div>
            </div>
          </div>
          <div className="h-56">
            <SavingsChart data={savingsChartData} formatCurrency={formatEuro} monthlySavings={displayedMonthlySavings} chartsReady={chartsReady} locale={locale} />
          </div>
        </div>
          ...
          {categoriesData.slice(0, 6).map((cat, index) => (
            <div key={index} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-baseline gap-2">
                <p className="text-2xl font-bold text-gray-900">{formatEuro(cat.montant)}</p>
              </div>
