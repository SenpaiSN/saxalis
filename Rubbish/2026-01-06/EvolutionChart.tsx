...
export default function EvolutionChart({ data, formatCurrency, chartsReady = true, locale }: Props) {
  ...
  const xTickProps = data.length > 6 ? { fontSize: 12, angle: -30 as const, textAnchor: 'end' as const } : { fontSize: 12 };
          ...
          <XAxis
            ...
            tickFormatter={(idx:any) => {
              ...
              if (typeof idx === 'number' && data[idx]?.date) {
                try { return data[idx].date.toLocaleDateString(usedLocale, { month: 'short', year: 'numeric' }); } catch (e) {}
              }
