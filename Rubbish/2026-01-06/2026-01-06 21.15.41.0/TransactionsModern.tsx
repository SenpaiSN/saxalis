...
export default function TransactionsModern({ transactions,
  ...
  // DEBUG: bandeau de debug pour vérifier le rendu et le nombre de transactions reçues sur mobile
  return (
    <div style={{background: 'orange', color: 'black', padding: 8, textAlign: 'center', zIndex: 9999}}>
      DEBUG: TransactionsModern monté, transactions: {transactions.length}
    </div>
  );

  // Filtrer les transactions
  // Include future transactions in the displayed list (Transactions tab shows all),
  // but keep a 'real-only' subset for indicators (stats use only real transactions)
  let __dbgCount = 0;
