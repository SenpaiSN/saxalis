...
export default function FixedVsVariableExpensesCard({ 
  ...
  const expenseData = useMemo(() => {
    ...
    // LOG DEBUG: Filtres et transactions reçues
    console.log('[DEBUG] Filtres:', { annee, mois, filterYear, filterMonth });
    console.log('[DEBUG] Transactions reçues:', transactions.map(t => ({ id: t.id, date: t.date, montant: t.montant, type: t.type, is_fixed: t.is_fixed })));

    const expensesForDisplay = transactions.filter(t => {
      if (t.type !== 'dépense') return false;
      // Normalisation date pour éviter timezone
      const txDate = new Date(t.date + 'T00:00:00');
      const txYear = txDate.getFullYear();
      const txMonth = txDate.getMonth() + 1;
      // LOG DEBUG: Détail parsing date
      console.log('[DEBUG] Transaction', t.id, 'date:', t.date, '->', txDate, 'txYear:', txYear, 'txMonth:', txMonth);

      if (filterYear === 'Tous' && filterMonth === 'Tous') {
        // Toutes les transactions de l'année en cours
        return txYear === currentYear;
      } else if (filterYear === 'Tous' && filterMonth !== 'Tous') {
        // Tous les mois = mois sélectionné, toutes années
    ...
    console.log('Transactions filtrées:', expensesForDisplay);
