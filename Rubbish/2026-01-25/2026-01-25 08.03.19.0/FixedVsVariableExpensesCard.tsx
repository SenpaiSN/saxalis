...
export default function FixedVsVariableExpensesCard({ 
  ...
  const expenseData = useMemo(() => {
    ...
    // Log de debug : aperçu des transactions reçues et des filtres
    console.log('[DEBUG] Aperçu transactions reçues:', transactions.slice(0, 10).map(t => ({
      id: t.id,
      date: t.date,
      type: t.type,
      montant: t.montant,
    ...
    console.log('[DEBUG] Filtres appliqués:', { annee, mois, filterYear, filterMonth, currentYear });
    ...
    const expensesForDisplay = transactions.filter(t => {
      ...
      // Log de debug pour chaque transaction potentielle
