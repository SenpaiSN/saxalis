...
export default function FixedVsVariableExpensesCard({ 
  ...
  const isIdealRatio = Math.abs(expenseData.fixedPercent - 60) < 10;
  const tooManyFixed = expenseData.fixedPercent > 70;

  // Ratio idéal unique pour tout le composant
  const IDEAL_FIXED = 60;
  const IDEAL_VARIABLE = 40;
  ...
  function getSmartAdvice(fixedPercent: number, variablePercent: number, margin: number) {
    if (fixedPercent > IDEAL_FIXED + 20) {
      return (
        <><span className="font-semibold">⚠️ Charges fixes très élevées :</span> Essayez de réduire certains abonnements ou charges récurrentes pour retrouver de la flexibilité. <br/> <span className="font-medium">Repère idéal : {IDEAL_FIXED}% fixes / {IDEAL_VARIABLE}% variables.</span></>
      );
    } else if (fixedPercent > IDEAL_FIXED) {
      return (
        <><span className="font-semibold">✅ Proche de l’idéal :</span> {IDEAL_FIXED}% fixes / {IDEAL_VARIABLE}% variables. Continuez à surveiller vos charges fixes.</>
      );
    } else if (fixedPercent >= IDEAL_FIXED - 10) {
      return (
        <><span className="font-semibold">👍 Budget équilibré :</span> Vous avez une bonne marge de manœuvre sur vos dépenses variables. <br/> <span className="font-medium">Repère idéal : {IDEAL_FIXED}% fixes / {IDEAL_VARIABLE}% variables.</span></>
      );
    } else if (fixedPercent < IDEAL_FIXED - 10 && variablePercent > IDEAL_VARIABLE + 10) {
      return (
        <><span className="font-semibold">⚠️ Dépenses variables importantes :</span> Essayez de mieux planifier vos achats ou de fixer des limites pour certains loisirs. <br/> <span className="font-medium">Repère idéal : {IDEAL_FIXED}% fixes / {IDEAL_VARIABLE}% variables.</span></>
      );
    } else if (fixedPercent < IDEAL_FIXED - 10) {
      return (
        <><span className="font-semibold">💡 Charges fixes faibles :</span> Attention à ne pas trop augmenter vos dépenses variables pour garder le contrôle de votre budget. <br/> <span className="font-medium">Repère idéal : {IDEAL_FIXED}% fixes / {IDEAL_VARIABLE}% variables.</span></>
      );
    } else {
      return (
        <><span className="font-semibold">ℹ️ Idéal :</span> {IDEAL_FIXED}% fixes / {IDEAL_VARIABLE}% variables. Vous avez une marge de manœuvre de <span className="font-bold">{formatCurrency(margin)}</span> sur votre budget.</>
      );
    }
  }
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {isIdealRatio && (
            <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full font-medium ml-2">
              ✓ Équilibré (idéal : {IDEAL_FIXED}% / {IDEAL_VARIABLE}%)
            </span>
          )}
        </div>
