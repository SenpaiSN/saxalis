...
export default function FixedVsVariableExpensesCard({ 
  ...
  // DEBUG: Afficher les transactions reçues
  console.log('Transactions reçues:', transactions);
  // DEBUG: Afficher les 5 premiers id_type et date
  console.log('Exemple id_type/date:', transactions.slice(0, 5).map(t => ({ id_type: t.id_type, date: t.date })));
  const expenseData = useMemo(() => {
    ...
    expensesForDisplay.forEach(tx => {
      ...
      const isFixed = tx.is_fixed === 1 || tx.is_fixed === "1" || tx.is_fixed === true;
      if (isFixed) {
        totalFixed += amount;
      } else {
