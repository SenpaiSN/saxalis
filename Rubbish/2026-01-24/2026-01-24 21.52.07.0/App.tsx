...
function App() {
      ...
      try {
        ...
        if (session.ok && session.data && session.data.success) {
          ...
          if (mounted) {
            ...
            // Persist and dispatch currency preference from server so UI is coherent across sessions (including private mode)
            try {
              const serverCurrency = session.data.user?.currency;
              if (serverCurrency) {
                try { localStorage.setItem('currency', serverCurrency); } catch {}
                try { window.dispatchEvent(new CustomEvent('preferences:currencyChanged', { detail: { currency: serverCurrency } })); } catch {}
              }
            } catch (e) { /* ignore */ }
          }
          await loadData();
  ...
  // Refresh data when user changes locale or currency preferences
  ...
  useEffect(() => {
    const handler = async (e: any) => {
      ...
      if (isAuthenticated) {
        ...
        try { await loadData(); } catch (err) { console.warn('refresh after currency change failed', err); }
        setRefreshingCurrency(false);
      } else {
        try { const stored = localStorage.getItem('transactions'); if (stored) setTransactions(JSON.parse(stored)); } catch {}
      }
    };
    const handlerLocale = async (e: any) => {
      const locale = e?.detail?.locale;
      if (!locale) return;
      if (isAuthenticated) {
        setRefreshingCurrency(true);
        try { await loadData(); } catch (err) { console.warn('refresh after locale change failed', err); }
        setRefreshingCurrency(false);
      } else {
        try { const stored = localStorage.getItem('transactions'); if (stored) setTransactions(JSON.parse(stored)); } catch {}
      }
    ...
    window.addEventListener('preferences:currencyChanged', handler as EventListener);
    window.addEventListener('preferences:localeChanged', handlerLocale as EventListener);
    return () => { window.removeEventListener('preferences:currencyChanged', handler as EventListener); window.removeEventListener('preferences:localeChanged', handlerLocale as EventListener); };
  }, [isAuthenticated]);
  ...
  const editTransaction = async (payload: { id: string; Date: string; Type: string; Catégorie: string; SousCatégorie: string; Montant: number; Notes?: string }) => {
      ...
      if (res.ok && res.data && res.data.success) {
        setTransactions(prev => {
          const next = prev.map(t => t.id === payload.id ? {
            ...
            montant: payload.Montant,
            date: payload.Date,
            type: payload.Type === 'expense' ? 'dépense' : (payload.Type === 'income' ? 'revenu' : (payload.Type === 'epargne' ? 'epargne' : 'revenu')),
    ...
    setTransactions(prev => {
      const next = prev.map(t => t.id === payload.id ? { ...t,
        ...
        type: payload.Type === 'expense' ? 'dépense' : (payload.Type === 'income' ? 'revenu' : (payload.Type === 'epargne' ? 'epargne' : 'revenu')),
        categorie: payload.Catégorie,
        note: payload.Notes || '' } : t) as Transaction[];
      localStorage.setItem('transactions', JSON.stringify(next));
      return next;
    });
  };
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      {refreshingCurrency && (
        <div className="w-full bg-yellow-50 border-b border-yellow-100 text-sm text-yellow-800 p-2 text-center">Actualisation des données en cours…</div>
      )}
        ...
        <main className="flex-1 overflow-auto">
          ...
          {activeTab === 'transactions' && (
            <TransactionsModern
              ...
              recherche={recherche} setRecherche={setRecherche}
              annee={annee} setAnnee={setMois} mois={mois} setMois={setMois}
              filtreType={filtreType} setFiltreType={setFiltreType}
              categorie={categorie} setCategorie={setCategorie} sousCategorie={sousCategorie} setSousCategorie={setSousCategorie}
              types={types} categories={categories} subcategories={subcategories}
              categoriesLoading={categoriesLoading} subcategoriesLoading={subcategoriesLoading}
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 z-50 shadow-sm" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
          <div className="flex justify-around items-center">
            ...
            <button
              ...
              className={`flex flex-col items-center gap-1 ${activeTab === 'stats' ? 'text-blue-600' : 'text-gray-400'}`}
            >
              <TrendingUp size={24} />
            ...
            <button
              ...
              className={`flex flex-col items-center gap-1 ${activeTab === 'objectifs' ? 'text-blue-600' : 'text-gray-400'}`}
            >
              <Target size={24} />
            ...
            <button
              ...
              className={`flex flex-col items-center gap-1 ${activeTab === 'profil' ? 'text-blue-600' : 'text-gray-400'}`}
            >
              <Settings size={24} />
          ...
          <LoginModal open={showLoginModal} onClose={() => setShowLoginModal(false)} onSuccess={async () => {
            ...
            try {
              ...
              if (session.ok && session.data && session.data.success) {
                setCurrentUser(session.data.user);
                setIsAuthenticated(true);
                // Persist and dispatch server currency so UI updates immediately after login
                try {
                  const serverCurrency = session.data.user?.currency;
                  if (serverCurrency) {
                    try { localStorage.setItem('currency', serverCurrency); } catch {}
