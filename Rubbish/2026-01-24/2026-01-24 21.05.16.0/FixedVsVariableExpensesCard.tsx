...
interface Transaction {
  ...
  montant: number;
  date: string;
  is_fixed?: number; // 0 ou 1, exposé par l'API
}
...
const DEFAULT_FIXED_THRESHOLD = 0.55; // 15% de variation = fixe
const MIN_MONTHS_FOR_CLASSIFICATION = 3; // Minimum de mois pour classifier
...
export default function FixedVsVariableExpensesCard({ 
  ...
  const expenseData = useMemo(() => {
    ...
    categoryMonthMap.forEach((monthData, category) => {
      // Si au moins une transaction de cette catégorie a is_fixed=1, on force à true
      const txs = expensesForClassification.filter(t => t.categorie === category);
      if (txs.some(t => t.is_fixed === 1)) {
    ...
    expensesForDisplay.forEach(tx => {
      const amount = Math.abs(tx.montant);
      // Priorité à is_fixed si présent
      const isFixed = typeof tx.is_fixed !== 'undefined' ? tx.is_fixed === 1 : (categoryClassification.get(tx.categorie) ?? false);
      if (isFixed) {
        totalFixed += amount;
      } else {
