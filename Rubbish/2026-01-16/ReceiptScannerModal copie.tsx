...
export default function ReceiptScannerModal({ onClose, onComplete, inline = false }: Props) {
  ...
  const runOCRAndExtract = async (dataUrl: string) => {
    ...
    try {
      ...
      // REMOVED AUTO-APPLY: User must now review and confirm
      // Show candidates if score is low or multiple candidates exist
      if (res.candidates && res.candidates.length > 1 && res.best && res.best.score100 < 80) {
    ...
    <div className="bg-white rounded-2xl shadow-xl w-full p-6">
      ...
      {!selectedImage ? (
        ...
        <div className="space-y-4">
          ...
          {extracted && !showCandidates && !ocrError && (
              ...
              <div className="flex items-center gap-3 text-green-600 bg-green-50 border border-green-200 rounded-xl p-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-green-700 mt-1">
                    Montant: <strong>{extracted.amount.toFixed(2)} €</strong> • 
                    Marchand: <strong>{extracted.merchant || 'Non détecté'}</strong>
                  </p>
                </div>
              </div>
